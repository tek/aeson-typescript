...
getTSFile tsDeclarations typesAndVals = "blah"
  where typeLines = ["blah" | (index, (typ, val)) <- zip [(1 :: Int)..] typesAndVals]
...
ensureTSCExists = doesFileExist localTSC >>= \exists -> unless exists $ void $ do
  ...
  when (exitCode /= ExitSuccess) $ putStrLn "blah"
